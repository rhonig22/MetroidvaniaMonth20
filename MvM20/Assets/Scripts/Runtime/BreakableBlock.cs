...
public class BreakableBlock : MonoBehaviour
{
    ...
    public void RenderCounters()
    {
        ...
        for (int i = 0; i < breakSize; i++)
        {
            ...
            if (breakSize < 4 || i == 0 || i == breakSize - 1)
            {
            ...
            {
                if (i == 1)
                    obj.transform.localPosition = new Vector3(-.5f + step, .5f - 4 * step, -1f);
                else if (i == 2)
                    obj.transform.localPosition = new Vector3(-.5f + 4 * step, .5f - step, -1f);
            }
